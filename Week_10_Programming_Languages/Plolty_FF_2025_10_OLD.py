...
lang_list = sorted(list(df.columns)[1:-1])
print(f'{lang_list = }')
...
app.layout = dbc.Container([
    ...
    dbc.Row(
        [
            dcc.Dropdown(       
            # prog_lang_list,       # list of choices
            ...
            ['Python', 'Perl'],   # defaults
            id='lang_dropdown',
            options=[{'label': lang, 'value': lang} for lang in lang_list],
            multi=True,
            value=[],
            ),
            ...
            dag.AgGrid(
                id='data_table',
                rowData=[{'Date': date, **{lang: df[lang][i] for lang in lang_list}} for i, date in enumerate(df['DATE'])],
                columnDefs=[
                    {'field': 'Date'},
