...
dmc_select = (
    dmc.Select(
       ...
       #  dmc.Text(id='dmc_select_parameter'),
)
...
#----- CALLBACK FUNCTIONS ------------------------------------------------------
def get_data_plot(plot_parameter):
    ...
    fig.update_layout(
        ...
        yaxis=dict(title=dict(text=plot_parameter))
    )
    # if group_by == 'YEAR':
    #     int_years = sorted(
...
def get_norm_plot(plot_parameter):
    df_plot = (
        df_global
        .select('FILM', 'FRANCHISE', 'YEAR', 'SERIES_NUM', plot_parameter)
        .pivot(
...
app.layout =  dmc.MantineProvider([
    ...
    dmc.Text('Marvelous (and not so Marvelous) Sequels', ta='center', style=style_h2),
...
def update_dashboard(parameter_name):
    ...
    return (
        get_data_plot(parameter_name),
        get_data_plot(parameter_name),   # temporary, replace wth get_data_norm
